...
OUTPUT_DIR="../json_output/nodes_list"
...
while IFS=',' read -r CLUSTER_ID CLUSTER_NAME || [[ -n "$CLUSTER_ID" ]]; do
    ...
    RESPONSE=$(curl --silent --location \
        --request GET \
        --url "https://api.cast.ai/v1/kubernetes/external-clusters/$CLUSTER_ID/nodes?nodeStatus=node_status_unspecified&lifecycleType=lifecycle_type_unspecified" \
